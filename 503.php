...
.top-logo img{
  width:120px;
  filter:drop-shadow(0 0 22px rgba(255,0,0,.9));
}
...
.center-box{
  ...
  box-shadow:0 0 55px rgba(255,0,0,.6);
...
<div class="center-box">
  ...
  <h1>503</h1>
  <p><b>Service Unavailable</b></p>
  <p>Sunucu şu anda geçici olarak hizmet veremiyor.<br>
     Lütfen birkaç dakika sonra tekrar dene.</p>
...
function draw(){
  ...
  for(const p of particles){
    ...
    ctx.fill();
    p.x+=p.dx; p.y+=p.dy;
    if(p.x<-150||p.x>w+150)p.dx*=-1;
    if(p.y<-150||p.y>h+150)p.dy*=-1;
  }
